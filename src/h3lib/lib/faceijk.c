...
/** @brief icosahedron face centers trigonometry parameters */
static const GeoCoordTrigon faceCenterGeoTrigon[NUM_ICOSA_FACES] = {
    {0.69413220800502795082564944095793, 0.71984753789261834988622013042914,
     0.94847807151851770690598186774878,
     0.31684278096323048590221560516511},  // face 0
    {0.26002533789655174478738786092435, 0.96560179352142050390739314025268,
     0.56847222717148337967074667176348,
     -0.82270245346339732073204231710406},  // face 1
    {0.49344409956464674316123364405939, 0.86977751212872533859155055324663,
     -0.97517663644548024937108721132972,
     0.22142838059038344455942137756210},  // face 2
    {0.82522741678320610247965305461548, 0.56480059365170331986405471980106,
     -0.43550924329289275593879438019940,
     0.90018425836405957518593368149595},  // face 3
    {0.88152423586898698282254827063298, 0.47213877364139311687551980867283,
     0.39124882757641737951459504074592,
     0.92028493137727662709579590227804},  // face 4
    {0.98511659246540528300783989834599, 0.17188746100093665880592652683845,
     0.99424346024862708670610800254508,
     -0.10714449007222327792110405653148},  // face 5
    {0.82197317966978000747246824175818, 0.56952619948826888052195727141225,
     0.18656965018188409466404209524626,
     -0.98244173650706101419416427233955},  // face 6
    {0.91005876017408837697075796313584, 0.41447925524735379498864062952634,
     -0.94983767542665575867033567192266,
     -0.31274332980910563017928893714270},  // face 7
    {0.99687590246953527994833166303579, -0.07898376463267356428765708642459,
     -0.66942129386756454856310938339448,
     0.74288298628833593362230658385670},  // face 8
    {0.97344671151384332130618304290692, -0.22891373886878077503759243427339,
     0.48511579860789583173996675213857,
     0.87444991963006291069149256145465},  // face 9
    {0.99687590246953527994833166303579, 0.07898376463267367530995954894024,
     0.66942129386756454856310938339448,
     -0.74288298628833582260000412134104},  // face 10
    {0.97344671151384321028388058039127, 0.22891373886878088605989489678905,
     -0.48511579860789599827342044591205,
     -0.87444991963006279966919009893900},  // face 11
    {0.98511659246540539403014236086165, -0.17188746100093654778362406432279,
     -0.99424346024862708670610800254508,
     0.10714449007222316689880159401582},  // face 12
    {0.82197317966978022951707316678949, -0.56952619948826865847735234638094,
     -0.18656965018188417793076894213300,
     0.98244173650706090317186180982389},  // face 13
    {0.91005876017408837697075796313584, -0.41447925524735373947748939826852,
     0.94983767542665564764803320940700,
     0.31274332980910574120159139965835},  // face 14
    {0.82522741678320621350195551713114, -0.56480059365170331986405471980106,
     0.43550924329289270042764314894157,
     -0.90018425836405957518593368149595},  // face 15
    {0.88152423586898698282254827063298, -0.47213877364139300585321734615718,
     -0.39124882757641721298114134697244,
     -0.92028493137727673811809836479370},  // face 16
    {0.69413220800502783980334697844228, -0.71984753789261834988622013042914,
     -0.94847807151851770690598186774878,
     -0.31684278096323059692451806768076},  // face 17
    {0.26002533789655163376508539840870, -0.96560179352142061492969560276833,
     -0.56847222717148360171535159679479,
     0.82270245346339720970973985458841},  // face 18
    {0.49344409956464685418353610657505, -0.86977751212872533859155055324663,
     0.97517663644548024937108721132972,
     -0.22142838059038333353711891504645},  // face 19
};
...
void _geoToGeoCoordTrigon(const GeoCoord* p, GeoCoordTrigon* trigon) {
    double lat = M_PI_2 - p->lat;
    double lon = p->lon;
...
void _geoToHex2d(const GeoCoord* g, int res, int* face, Vec2d* v) {
    ...
    *face = 0;
    GeoCoordTrigon t;
    _geoToGeoCoordTrigon(g, &t);
    // double r = _geoDistRads(&faceCenterGeo[0], g);
    double r = _geoDistRadsCX(&faceCenterGeoTrigon[0], &t);
    // if (r - _geoDistRadsCX(&faceCenterGeoTrigon[0], &t) > 0.000000001) {
    //     printf("Index %d %.32f %s\n", 0,
    //            fabs(r - _geoDistRadsCX(&faceCenterGeoTrigon[0], &t)), "Not
    //            EQUAL");
    // }
    for (int f = 1; f < NUM_ICOSA_FACES; f++) {
        // double dist = _geoDistRads(&faceCenterGeo[f], g);

        // if (dist - _geoDistRadsCX(&faceCenterGeoTrigon[f], &t) > 0.000000001)
        // {
        //     printf("Index %d %.32f %s\n", f,
        //            fabs(dist - _geoDistRadsCX(&faceCenterGeoTrigon[f], &t)),
        //            "Not EQUAL");
        // }
        double dist = _geoDistRadsCX(&faceCenterGeoTrigon[f], &t);
        if (dist < r) {
            *face = f;
            r = dist;
        }
